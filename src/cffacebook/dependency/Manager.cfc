component {
  ...
  function init(localRepo=getDirectoryFromPath(getMetadata(this).path) & "/repo", repositories="default", javatools = true) {
...
//    return new "#paths[dependency]#.#componentName#" (argumentColection = args);
  ...
  public function simpleResolve(required dependencies, Boolean transitive = true, scope="runtime") {
    ...
    if(!fileExists(filePath) || !hashMatch(filePath)) {
      ...
      var artifactFile = getRemoteArtifact(dependencies);
...
//      request.debug(messages);
      throw(message="could not get #filePath# or hash incorrect",detail=messages.toString());
    }
    return filePath;
  }
